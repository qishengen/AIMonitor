...
Code/host/tests/           回环自测：串口模拟 14/14 + TCP 模拟(TcpSimServer) 6/6
...
2. **Web 控制面板**：手机/PC 连热点 `AIMonitor-xxxx`（**热点固定开放无密码，直接连接**）→ 浏览器开 `192.168.4.1` → 状态卡片（网络/存储/播放器/系统）+ 文件播放/删除 + 待机/主页/重启/恢复出厂/重连WiFi 按钮 + 配网表单（选 WiFi 填密码保存 → 切路由器，STA 失败 15s 自动退回热点）。门户 AP/STA 任何模式均可达。
...
- 固件 IRAM 92%（WiFi 代码全进 Flash 没爆）、RAM 69%、Flash 31%，加代码需留意。
