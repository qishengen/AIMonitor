...
typedef void (*idle_info_cb)(bool blink);                            // IDLE 待机信息屏（IP/端口等），blink 为呼吸指示

void player_init(frame_cb render, event_cb ev);
void player_set_idle_info_cb(idle_info_cb cb);  // 注册后 IDLE 改画信息屏，替代内置待机动画
...
bool player_play_home(void);  // 播放内置主页动画(idle_anim)，直到 stop
